...
interface ActiveMembersTableProps {
  data: GetParticipantsOverviewResponse["data"]["participants"];
}

export const ActiveMembersTable: FC<ActiveMembersTableProps> = ({ data }) => {
  const columns: ColumnDef<
    GetParticipantsOverviewResponse["data"]["participants"][number]
  >[] = [
    {
      accessorKey: "player",
      header: "Player",
    ...
    },
    {
      ...
      header: "Joined At",
      cell: ({ row }) =>
        dayjs(row.original.joinedAt).format("MMM DD, YYYY - HH:mm"),
    },
  ];
